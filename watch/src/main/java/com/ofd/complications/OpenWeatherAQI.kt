...
class OpenWeatherAQI : ComplicationDataSourceService() {
    ...
    override fun onComplicationRequest(
        ...
        if (api == null) api = OpenWeatherAQIService.OpenWeatherAQIAPI(applicationContext)
        ...
        if (wl.valid) {
            ...
            updateComplication(
                ...
                listener,
                OpenWeatherAQIService.OWAQIResult("no location", 0, 0, 0f, false)
            )
        }
    }
...
//                    color = 5
                ...
                RangedValueComplicationData.Builder(
                    ...
                    contentDescription = PlainComplicationText.Builder(
                        (if (aqi.hasRealData) value.toInt()
                            .toString() + " ppm" else "--") + "?Color=" + color + "&ExpiresMS=" + (System.currentTimeMillis() + 30 * 60 * 1000)
                    ).build()
                ).setMonochromaticImage(image).build()
