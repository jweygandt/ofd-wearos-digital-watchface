...
import com.odbol.wear.airquality.purpleair.CachingClient
...
class OpenWeatherAQIService {
    ...
    class OWAQIResult(val fulljson: String, val date: Long, val aqi: Float, val hasRealData: Boolean)
...
//                Log.d(TAG, "main: " + main.toString())
    ...
    class OpenWeatherAQIAPI(context: Context) {
        companion object {
            ...
            fun statusString() =
                metricNumCalls.get()
                    .toString() + "/" + (metricNumSuccess.get() + metricNumBypass.get()) +
                    ":" + metricNumBypass.get() + ":" +
            ...
            call.enqueue(object : Callback<OWAQIResult> {
                override fun onResponse(call: Call<OWAQIResult>, response: Response<OWAQIResult>) {
                    ...
                    if (response.isSuccessful && response.body() != null) {
                        ...
                        Log.d(TAG, "Issues: " + response.message() + ":" + response.body() + ":"
                            + response.code())
